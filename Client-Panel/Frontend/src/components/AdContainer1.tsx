...
const AdContainer1 = () => {
  ...
  return (
<div className="relative w-full h-[80vh]  bg-gradient-to-br from-[#131313] via-[#1a1a1a] to-[#2b2b2b] rounded-3xl overflow-hidden">
  {/* Carousel Container */}
  <div
  ...
  >
      ...
      {loopedBanners.map((url, idx) => (
        ...
        >
          ...
          {idx === current && (
            <>
              <button
                className="absolute left-4 top-1/2 transform -translate-y-1/2 text-white hover:text-purple-300 bg-transparent  transition-colors p-2 rounded-full cursor-pointer z-10"
                onClick={handlePrev}
                aria-label="Previous banner"
              >
                <VscRunAll className="text-2xl scale-x-[-1]" />
              </button>
              <button
                className="absolute right-4 top-1/2 transform -translate-y-1/2 text-white hover:text-purple-300 bg-tansparent transition-colors p-2 rounded-full cursor-pointer z-10"
                onClick={handleNext}
                aria-label="Next banner"
              >
                <VscRunAll className="text-2xl" />
              </button>
            </>
          )}
        </div>
    ...
    {/* Dots */}
    <div className="absolute bottom-1 w-full flex justify-center gap-3 z-20">
      {banners.map((_, idx) => {
