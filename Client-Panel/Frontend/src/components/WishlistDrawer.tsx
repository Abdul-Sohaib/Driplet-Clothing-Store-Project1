  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end ">
      <div
        ...
        onClick={onClose}
      ></div>
      <div className="w-full max-w-md inset-0 bg-[#F5F5DC] h-full shadow-lg flex flex-col z-50 rounded-md border border-black">
        <div className="flex justify-between items-center p-4 border-b-2 border-black">
          <h2 className="text-xl font-bold textheading text-black uppercase">Your Wishlist</h2>
          <button onClick={onClose} className="text-2xl text-red-300 hover:text-red-400 cursor-pointer">
            <IoClose />
          </button>
        </div>
        <div className="flex-1 overflow-y-auto p-4 ">
          ...
          {!loading && !error && wishlist.length === 0 && (
            <div className="text-center text-black  navfonts font-semibold text-lg  flex justify-center items-center uppercase  flex-col gap-10">Your wishlist is empty.
         <img src={emplywish} alt="" className="w-60"/>
            </div>
          )}
          {!loading &&
            !error &&
            wishlist.map((item) => (
              <div
                ...
                className="flex items-center gap-4 p-4 border-b-1 border-black"
              >
                <img
                  src={item.product.imageUrls[0] || "https://via.placeholder.com/100"}
                  alt={item.product.name}
                  className="w-20 h-20 object-cover rounded-md border border-purple-300 text-sm flex "
                />
                <div className="flex-1">
                  <h3 className="text-sm font-semibold navfonts text-black line-clamp-2">{item.product.name}</h3>
                  <p className="text-sm text-gray-600">Size: {item.size}</p>
                  <p className="text-sm font-bold text-black">₹{item.product.price}</p>
                </div>
                <div className="flex flex-col gap-2">
                  ...
                  <button
                    className="text-sm text-red-500 hover:text-red-700 navfonts cursor-pointer"
